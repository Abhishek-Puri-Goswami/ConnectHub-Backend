...
class LoggingAspectTest {
    ...
    void logServiceCall_returnsResult() throws Throwable {
        when(joinPoint.proceed()).thenReturn("ONLINE");
        when(joinPoint.getTarget()).thenReturn(new Object());
        when(joinPoint.getSignature()).thenReturn(signature);
        when(signature.getName()).thenReturn("getStatus");
        when(joinPoint.getArgs()).thenReturn(new Object[]{1});

        assertEquals("ONLINE", aspect.logServiceCall(joinPoint));
    }
    ...
    void logServiceCall_noArgs_ok() throws Throwable {
        ...
        when(joinPoint.getTarget()).thenReturn(new Object());
        when(joinPoint.getSignature()).thenReturn(signature);
        when(signature.getName()).thenReturn("clearAll");

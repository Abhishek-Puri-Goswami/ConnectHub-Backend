...
class ExceptionAspectTest {
    ...
    void logException_logsWithoutThrowing() {
        ...
        when(signature.getName()).thenReturn("saveMessage");
        when(joinPoint.getArgs()).thenReturn(new Object[]{"msg"});

        aspect.logException(joinPoint, new RuntimeException("message error"));
    ...
    void logException_nullMessage_handlesGracefully() {
        when(joinPoint.getTarget()).thenReturn(new Object());
        when(joinPoint.getSignature()).thenReturn(signature);
        when(signature.getName()).thenReturn("deleteMessage");

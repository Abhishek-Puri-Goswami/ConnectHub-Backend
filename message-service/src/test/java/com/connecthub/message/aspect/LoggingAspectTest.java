...
class LoggingAspectTest {
    ...
    void logServiceCall_returnsResult() throws Throwable {
        when(joinPoint.proceed()).thenReturn("message");
        when(joinPoint.getTarget()).thenReturn(new Object());
        when(joinPoint.getSignature()).thenReturn(signature);
        when(signature.getName()).thenReturn("getMessage");
        when(joinPoint.getArgs()).thenReturn(new Object[]{"m1"});

        assertEquals("message", aspect.logServiceCall(joinPoint));
    }
    ...
    void logServiceCall_noArgs_ok() throws Throwable {
        ...
        when(joinPoint.getTarget()).thenReturn(new Object());
        when(joinPoint.getSignature()).thenReturn(signature);
        when(signature.getName()).thenReturn("listMessages");

...
class PerformanceAspectTest {
    ...
    void monitorPerformance_returnsResult() throws Throwable {
        when(joinPoint.proceed()).thenReturn("order_1");

        assertEquals("order_1", aspect.monitorPerformance(joinPoint));
    }
    ...
    void monitorPerformance_propagatesException() throws Throwable {
        when(joinPoint.proceed()).thenThrow(new RuntimeException("payment error"));
